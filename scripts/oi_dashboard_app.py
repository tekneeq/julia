...
# ---------------------------------------------------------------------------
# Today's move prediction from past sigma-multiples
# ---------------------------------------------------------------------------

def _sigma_prediction(history: list[dict], today: date) -> dict | None:
    """Predict how many implied sigmas today will move, from how past
    sessions moved relative to *their* implied bands.

    Model: each completed session gets a sigma-multiple
    ``z = actual% / implied-1σ%`` — how much of its own pre-priced 1σ
    move the day actually realized. The recency-weighted RMS of those
    multiples, ``s``, is the realized-to-implied vol ratio: s < 1 means
    the market has been overpricing moves lately (days keep landing
    inside the bands), s > 1 means moves keep running hotter than
    priced. Today's |move| is then modeled as |N(0, s²)| in sigma units,
    which gives P(within ±kσ) = 2Φ(k/s) − 1 for k = 1, 2, 3.

    The headline bucket is the smallest k with P ≥ 68.27% — the same
    confidence a 1σ band carries — which works out to k = ceil(s).

    Why RMS-of-z rather than a streak rule ("two calm days → calm
    day"): implied vol already reprices every night, so what persists
    day-to-day is the *ratio* of realized to implied, and the weighting
    (half-life = half the window) makes the last couple of sessions
    dominate exactly the way a streak intuition wants.

    Returns None when fewer than 3 completed sessions have both an
    implied band and an actual move.
    """
    ...
    # (day, sigma-multiple, actual %, implied 1σ %) per completed session
    zs = [
        (
            h["day"],
            h["actual_pct"] / h["implied"][conf1],
            h["actual_pct"],
            h["implied"][conf1],
        )
        for h in completed
    ...
    n = len(zs)
    half_life = max(n / 2.0, 2.0)
    weights = [0.5 ** ((n - 1 - i) / half_life) for i in range(n)]  # last = newest
    s2 = sum(w * row[1] * row[1] for w, row in zip(weights, zs)) / sum(weights)
    # Floor: a run of dead-flat sessions would otherwise predict ~zero
    # movement with absurd certainty.
    s = max(math.sqrt(s2), 0.05)

    probs = {k: 2.0 * float(norm.cdf(k / s)) - 1.0 for k in (1, 2, 3)}
    predicted = next((k for k in (1, 2, 3) if probs[k] >= 0.6827), None)
    ...
    out: dict = {
        ...
        "s": s,
        "probs": probs,
        "predicted": predicted,             # None → ">3σ likely"
        "exp_abs_z": s * math.sqrt(2.0 / math.pi),
        "z80": s * float(norm.ppf(0.9)),    # P(|z| ≤ z80) = 80%
        "today_implied1": None,
        "today_ref_spot": None,
        "today_z": None,
    ...
    if history and history[-1]["day"] == today:
        ...
        if t["implied"] is not None:
            ...
            if t["actual_pct"] is not None:
                out["today_actual_pct"] = t["actual_pct"]
                out["today_z"] = t["actual_pct"] / t["implied"][conf1]
    return out


def _z_bucket_color(z: float) -> str:
    """Same palette as the implied bands, keyed by which band |z| hit."""
    az = abs(z)
    if az <= 1:
        return "#1f77b4"
    if az <= 2:
...
def _render_sigma_prediction(ticker: str, history: list[dict]) -> None:
    today = date.today()
    pred = _sigma_prediction(history, today)
    if pred is None:
        st.info(
    ...
    # --- chart: past sessions in sigma units --------------------------
    days = [row[0] for row in pred["zs"]]
    zvals = [row[1] for row in pred["zs"]]
    labels = [f"{d.strftime('%b %d')} ({d.strftime('%a')})" for d in days]
    colors = [_z_bucket_color(z) for z in zvals]
    hover = [[row[2], row[3]] for row in pred["zs"]]
    ...
    fig.add_trace(go.Bar(
        ...
        customdata=hover,
        hovertemplate=(
            "<b>%{y:+.2f}σ</b>  (moved %{customdata[0]:+.2f}% vs "
            "±%{customdata[1]:.2f}% implied 1σ)<extra></extra>"
        ),
    ))
    ymax = max(2.3, max((abs(z) for z in zvals), default=0) + 0.5)
    if pred["today_z"] is not None:
        ymax = max(ymax, abs(pred["today_z"]) + 0.5)
        fig.add_trace(go.Bar(
            x=[f"{today.strftime('%b %d')} (today)"],
            y=[pred["today_z"]],
            name="Today so far (live)",
            marker=dict(
                color=_z_bucket_color(pred["today_z"]),
                opacity=0.45,
                line=dict(color="#333", width=2),
            ),
            width=0.5,
            customdata=[[pred["today_actual_pct"], pred["today_implied1"]]],
            hovertemplate=(
                "<b>%{y:+.2f}σ so far</b>  (moved %{customdata[0]:+.2f}% vs "
                "±%{customdata[1]:.2f}% implied 1σ)<extra></extra>"
            ),
        ))
    for k, color in ((1, "#1f77b4"), (2, "#ff7f0e"), (3, "#d62728")):
        if k <= ymax:
            fig.add_hline(y=k, line=dict(color=color, width=1, dash="dot"))
            fig.add_hline(y=-k, line=dict(color=color, width=1, dash="dot"))
    fig.add_hline(y=0, line_color="#999", line_width=1)
    fig.update_layout(
        title=(
            f"{ticker} — how past sessions moved, in multiples of their "
            f"own implied 1σ"
        ),
        xaxis_title="Session",
        yaxis_title="Actual move (σ multiples)",
        height=340,
        hovermode="x unified",
    ...
    # --- verdict cards -------------------------------------------------
    p1, p2, p3 = pred["probs"][1], pred["probs"][2], pred["probs"][3]
    col_a, col_b, col_c, col_d = st.columns(4)
    with col_a:
        if pred["predicted"] is not None:
            ...
            st.metric(
                ...
                f"within ±{k}σ",
                f"{pred['probs'][k] * 100:.0f}% est. probability",
                delta_color="off",
                help=(
                    "Smallest band that clears 68.27% estimated "
                    "probability — the same confidence a 1σ band "
            ...
            st.metric(
                "Today's predicted move",
                "beyond ±3σ",
                "recent sessions ran extremely hot",
                delta_color="off",
            )
    with col_b:
        st.metric(
            "Chance ≤ ±1σ",
            f"{p1 * 100:.0f}%",
            f"{(p1 - 0.6827) * 100:+.0f}pp vs fairly-priced",
            delta_color="off",
            help=(
                "A fairly-priced options market puts 68% here. Higher "
                "= recent sessions have been calmer than implied."
            ),
        )
    with col_c:
        st.metric(
            "Chance ≤ ±2σ",
            f"{p2 * 100:.0f}%",
            f"{(p2 - 0.9545) * 100:+.0f}pp vs fairly-priced",
            delta_color="off",
            help="A fairly-priced market puts 95.45% here.",
        )
    with col_d:
        st.metric(
            "Realized / implied ratio",
            f"{pred['s']:.2f}×",
            f"recency-weighted over {pred['n']} sessions",
            delta_color="off",
            help=(
                "RMS of the sigma-multiples above, weighted toward "
                "recent sessions (half-life = half the window). 1.00× "
                "= moves realizing exactly what options price in; "
                "<1 = vol overpriced; >1 = underpriced. Also P(≤3σ): "
                f"{p3 * 100:.1f}%."
            ),
        )

    # --- translate into today's % / $ ----------------------------------
    if pred["today_implied1"] is not None and pred["today_ref_spot"]:
        imp1 = pred["today_implied1"]
        ref = pred["today_ref_spot"]
        exp_pct = pred["exp_abs_z"] * imp1
        p80_pct = pred["z80"] * imp1
        lo80 = ref * (1 - p80_pct / 100.0)
        hi80 = ref * (1 + p80_pct / 100.0)
        live = ""
        if pred["today_z"] is not None:
            live = (
                f"  So far today: **{pred['today_actual_pct']:+.2f}%** "
                f"= **{pred['today_z']:+.2f}σ**."
            )
        st.caption(
            f"In price terms (today's 1σ = ±{imp1:.2f}% from the prior "
            f"session's ${ref:.2f}): expected |move| ≈ "
            f"**±{exp_pct:.2f}%** (≈ ${ref * exp_pct / 100.0:.2f}), and an "
            f"80% chance of closing within ±{p80_pct:.2f}% — roughly "
            f"**${lo80:.2f} – ${hi80:.2f}**.{live}"
        )
    else:
        st.caption(
            "No implied band for today yet (needs a snapshot from the "
            "prior session), so the prediction is shown in σ units only."
        )
...
# ---------------------------------------------------------------------------
st.divider()
st.header("🔮 Today's move — sigma prediction")

pred_window = st.selectbox(
    "Prediction lookback",
    options=[5, 10, 15, 20],
    index=1,
    format_func=lambda n: f"Based on past {n} sessions",
    key="pred_window",
    help=(
        "How many completed sessions feed the estimate. Short windows "
        "react fast but are noisy; long windows are steadier but slower "
        "to notice a regime change. Recent sessions always count more "
        "(exponential weighting, half-life = half the window)."
    ),
)
st.caption(
    "Each past session is scored in **sigma multiples**: its actual move "
    "divided by the ±1σ move options had priced in for it the evening "
    "before. The recency-weighted RMS of those multiples is the "
    "realized-to-implied ratio — if recent days kept landing inside "
    "their bands it drops below 1× and today is predicted calm; if "
    "days kept blowing through, it rises and the prediction widens. "
    "Scaling a normal curve by that ratio gives the probability that "
    "today closes within ±1σ / ±2σ / ±3σ of its own (frozen, "
    "prior-session) implied bands. Heuristic, not a guarantee — a "
    "surprise headline ignores every window length."
)

for ticker in tickers:
    st.markdown(f"**{ticker}**")
    pred_hist = _implied_vs_actual_history(
        ticker, pred_window + 1, _iva_end.isoformat()
    )
    _render_sigma_prediction(ticker, pred_hist)
